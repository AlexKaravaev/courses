...
time = 0.5;     //Время переходного процесса
...
Bbh = [3*w0+A(2,2); 3*w0^2+A(3,1); w0^3-A(2,2)*A(3,1)+A(2,1)*A(3,2)]
Kbh = Abh^-1*Bbh
...
//начальные условия
psi0 = %pi/18;     //начальный угол
dpsi0 = 0;      //начальная скорость падения
